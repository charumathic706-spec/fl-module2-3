...
type TamperAlert struct {
	ID int; Round int; AlertType, Detail, Severity string; Timestamp int64; TxID string
}
type AuditEvent struct {
	ID int; EventType string; Round int; Actor, Data string; Timestamp int64; TxID string
}
type VerifyResult struct{ Verified bool; Round int; StoredHash, ClaimedHash string }
...
func (t *TrustRegistry) nextID(ctx contractapi.TransactionContextInterface, ns string) (int, error) {
	...
	n := 0
	if b != nil { n, _ = strconv.Atoi(string(b)) }
	n++
...
func (t *TrustRegistry) RegisterModel(ctx contractapi.TransactionContextInterface,
	...
	if round == 1 {
		if prevBlockHash != "GENESIS" { return fmt.Errorf("round 1 must use GENESIS") }
	} else {
		pb, _ := ctx.GetStub().GetState(fmt.Sprintf("MODEL:%d", round-1))
		if pb == nil { return fmt.Errorf("round %d not found — register sequentially", round-1) }
		var prev ModelRecord
		json.Unmarshal(pb, &prev)
		if prev.BlockHash != prevBlockHash { return fmt.Errorf("CHAIN_VIOLATION round %d", round) }
	}
	f1, _ := strconv.ParseFloat(f1Str, 64)
	auc, _ := strconv.ParseFloat(aucStr, 64)
	pc, _ := strconv.Atoi(paramStr); tb, _ := strconv.Atoi(bytesStr)
...
func (t *TrustRegistry) GetModel(ctx contractapi.TransactionContextInterface, roundStr string) (*ModelRecord, error) {
	...
	if err != nil || b == nil { return nil, fmt.Errorf("round %s not found", roundStr) }
	var rec ModelRecord; json.Unmarshal(b, &rec); return &rec, nil
}
...
func (t *TrustRegistry) QueryAllModels(ctx contractapi.TransactionContextInterface) ([]*ModelRecord, error) {
	...
	var out []*ModelRecord
	for iter.HasNext() {
		kv, _ := iter.Next(); var rec ModelRecord
		if json.Unmarshal(kv.Value, &rec) == nil { out = append(out, &rec) }
	}
	return out, nil
}
...
func (t *TrustRegistry) RaiseTamperAlert(ctx contractapi.TransactionContextInterface, roundStr, alertType, detail, severity string) error {
	id, err := t.nextID(ctx, "ALERT"); if err != nil { return err }
	round, _ := strconv.Atoi(roundStr)
...
func (t *TrustRegistry) GetAlerts(ctx contractapi.TransactionContextInterface) ([]*TamperAlert, error) {
	...
	var out []*TamperAlert
	for iter.HasNext() {
		kv, _ := iter.Next(); var a TamperAlert
		if json.Unmarshal(kv.Value, &a) == nil { out = append(out, &a) }
	}
	return out, nil
}
...
func (t *TrustRegistry) AppendEvent(ctx contractapi.TransactionContextInterface, eventType, roundStr, actor, dataJSON string) error {
	id, err := t.nextID(ctx, "AUDIT"); if err != nil { return err }
	round, _ := strconv.Atoi(roundStr)
...
func (t *TrustRegistry) ExportAuditTrail(ctx contractapi.TransactionContextInterface) ([]*AuditEvent, error) {
	...
	var out []*AuditEvent
	for iter.HasNext() {
		kv, _ := iter.Next(); var ev AuditEvent
		if json.Unmarshal(kv.Value, &ev) == nil { out = append(out, &ev) }
	}
	return out, nil
}
